...
CREATE TABLE Db_user_date_time_event(
  id_user_date_time_event     INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, 
  name_work   TEXT,  
  id_user INTEGER, 
  id_date_time_event INTEGER,  
  FOREIGN KEY(id_user) REFERENCES Db_user(id_user)ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY(id_date_time_event) REFERENCES Db_date_time_event(id_date_time_event)ON DELETE CASCADE ON UPDATE CASCADE
);
